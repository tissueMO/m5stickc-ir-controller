...
- [HTTP/GET] スマホ (Web ブラウザー) => Web サーバー
    - 画面表示
- [HTTP/GET] スマホ (Web ブラウザー) => M5StickC
...
- ビルドしたフロント用コンテンツを配置している Web サーバーに疎通できるクライアント環境の Web ブラウザーから、フロント用コンテンツの index.html にアクセスします。
    - リモコン画面が表示されます。
    - 最新ではない Web ブラウザーや Internet Explorer 等では正しく動作しない可能性があります。
